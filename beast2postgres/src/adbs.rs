...
/// Must run this SQL DDL before...
/// ```sql
/// CREATE TABLE adsb_message (
///  icao24 INTEGER NOT NULL,
///  callsign TEXT,
///  altitude_ft INTEGER,
...
///  emergency TEXT,
///  wake_vortex TEXT,
///  pressure_setting_mbar REAL,
///  selected_heading_deg REAL
/// );
/// ```
...
pub struct AdsbMessage {
    ...
    // selected_altitude_ft: Option<u32>,
...
impl AdsbMessage {
    fn empty(icao24: ICAO) -> Self {
        AdsbMessage {
            // ICAO implements no ToSql; store the raw 24-bit address as INT4.
            icao24: icao24.0 as i32,
            timestamp: 0.0,
            callsign: None,
            altitude_ft: None,
            // squawk: None,
    ...
    pub fn from_frame(
        ...
        let out = match msg.df {
            ...
            SurveillanceIdentityReply { ap, fs, .. } => {
                let mut out = AdsbMessage::empty(ICAO(ap.0));
                // out.squawk = Some(id.0);
                out.on_ground = on_ground(fs);
                Some(out)
            }
            CommBIdentityReply { ap, fs, .. } => {
                let mut out = AdsbMessage::empty(ICAO(ap.0));
                // out.squawk = Some(id.0);
                out.on_ground = on_ground(fs);
                Some(out)
...
pub fn mlat_timestamp(msg: &[u8]) -> f64 {
...
/// Returns RSSI in dB.
pub fn beast_rssi(msg: &[u8]) -> Option<f32> {
    if msg[8] == 0xff {
        return None;
    }
